...
#数据集类: 适用于二分类任务，其中图像和掩膜是成对的，用于训练或评估模型
class binary_class(Dataset):
        ...
        def __getitem__(self,idx):
            ...
            img = io.imread(image_path)[:,:,:3].astype('float32') #选择前3个通道,加载的图像被转换为 float32 类型
            mask = io.imread(mask_path, as_gray=True)
            image_id = self.folders[idx] #文件夹名称作为标识符
            augmented = self.transforms(image=img, mask=mask) #将图像和掩膜传递给转换函数进行预处理
            img = augmented['image']
            mask = augmented['mask']
            
            return (img,mask,image_id)

class predict_binary_class(Dataset):
        def __init__(self, path, data, transform=None):
            self.path = path
            self.folders = data
            self.transforms = transform
        ...
        def __getitem__(self,idx):
            ...
            image_id = self.folders[idx]
            augmented = self.transforms(image=img) #将图像和掩膜传递给转换函数进行预处理
            img = augmented['image']
            return (img, image_id)
