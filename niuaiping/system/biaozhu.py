...
class LabelTool:
    def __init__(self):
        self.wildcard = ".png .jpg"      #文件过滤器(wildcard)
        page = Tk()     
        page.title('标记小工具')
        self.pathStr = StringVar()      #用于显示选择的图片路径 pathStr 为可变化的字符串
        
        #textvariable按钮显示的内容，它是可更改的；command是点击后调用的函数  pady指定控件上下边距的大小
        ...
        self.pathStr.set('点击按钮选择要标记的图片路径')    #设置字符串内容
